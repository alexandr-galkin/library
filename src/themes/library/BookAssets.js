...
export const BOOK_SIZES={small:{name:'Маленькие',height:104,width:72},medium:{name:'Средние',height:104,width:72},large:{name:'Большие',height:104,width:72}};
...
export function createBookSVG(book){
  ...
  return `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" class="book-art" aria-hidden="true">
    ...
    <g filter="url(#bookShadow${book.uid})">
      ...
      <text x="${w/2}" y="${Math.round(h*.30)}" text-anchor="middle" fill="rgba(247,226,180,.72)" font-size="8" font-family="Georgia,serif" letter-spacing="1.2">LIBRARY</text>
