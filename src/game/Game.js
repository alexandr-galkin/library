...
export class Game {
  constructor({ app = document.getElementById('app'), storage, documentRef = document, windowRef = globalThis } = {}) {
    ...
    this.visibilityHandler = () => this.handleVisibilityChange(); this.yandexPauseHandler = () => this.sound.pauseAudio(); this.yandexResumeHandler = () => this.sound.resumeAudio(); this.yandexAdOpenHandler = () => this.sound.pauseAudio(); this.yandexAdCloseHandler = () => { if (this.engine.isPlaying && !this.engine.isPaused) this.sound.resumeAudio(); };
    ...
    this.localeCleanup = onLocaleChanged(() => this.refreshLocale()); this.document.addEventListener('visibilitychange', this.visibilityHandler); this.window.addEventListener('yandex-game-pause', this.yandexPauseHandler); this.window.addEventListener('yandex-game-resume', this.yandexResumeHandler); this.window.addEventListener('yandex-ad-open', this.yandexAdOpenHandler); this.window.addEventListener('yandex-ad-close', this.yandexAdCloseHandler);
