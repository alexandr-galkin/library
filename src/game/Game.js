...
export class Game {
  constructor({ app = document.getElementById('app'), storage, documentRef = document, windowRef = globalThis } = {}) {
    ...
    this.engine = new GameEngine({
      app: this.app, documentRef: this.document, windowRef: this.window,
      state: this.state, stats: this.stats, timer: this.timer, sound: this.sound,
      particles: this.particles, levelManager: this.levelManager, theme: this.theme,
      layoutManager: this.layoutManager,
      actions: { onSettings: () => this.showSettingsFromGame(), onMenu: () => this.showMenu(), onLevelComplete: (level, score, bonus) => this.showLevelComplete(level, score, bonus) },
    });
    ...
    this.app.append(this.menu.container); this.document.body?.append(this.settings.container);
    this.menu.show(); this.settings.hide(); this.applySettings();
  ...
  /** Current level number. */ get currentLevel() { return this.engine.level?.id ?? 0; }
  /** Whether the game is paused. */ get isPaused() { return this.engine.isPaused; }
  /** Whether a transition is active. */ get isTransitioning() { return this.engine.isTransitioning; }
  /** Whether the game is active. */ get isInGame() { return this.engine.isActive; }
  ...
  /** Pause gameplay. */ pauseGame() { return this.engine.pause(); }
  /** Resume gameplay. */ resumeGame() { return this.engine.resume(); }
  ...
  /** Retry the current level. */ retryLevel() { return this.engine.retry(); }
  /** Advance to the next level. */ nextLevel() { return this.engine.next(); }
  ...
  destroy() { this.engine.destroy(); this.layoutCleanup?.(); this.themeCleanup?.(); this.document.removeEventListener('visibilitychange', this.visibilityHandler); this.timer.destroy(); this.particles.destroy(); this.sound.destroy(); this.menu.destroy(); this.settings.destroy(); this.levelComplete.destroy(); }
}
