...
export class GameRenderer {
  ...
  buildHUD() {
    ...
    const items = [
      [t("hud.level"), "hud-level", "1"],
      [t("hud.difficulty"), "hud-diff", "1"],
      [t("hud.moves"), "hud-moves", "0"],
      [t("hud.score"), "hud-score", "0"],
    ];
  ...
  buildGameZones() {
    ...
    this.elements.table.append(
      this.elements.objectsZone,
      this.elements.containersZone,
    );
  }
  buildOverlays() {
    this.elements.failOverlay = this.createOverlay(
      "fail-overlay",
      t("overlays.failTitle"),
      t("overlays.failText"),
      [
        ["retry", `↻ ${t("overlays.retry")}`, "menu-btn-primary"],
        ["menu", `← ${t("overlays.menu")}`, "menu-btn-secondary"],
      ],
    );
    this.elements.pauseOverlay = this.createOverlay(
      "pause-overlay",
      t("overlays.pauseTitle"),
      t("overlays.pauseText"),
      [
        ["resume", `▶ ${t("overlays.resume")}`, "menu-btn-primary"],
        ["settings", `⚙ ${t("overlays.settings")}`, "menu-btn-secondary"],
        ["menu", `← ${t("overlays.menu")}`, "menu-btn-secondary"],
      ],
    );
  }
  ...
  setRule(text) {
    this.elements.ruleText.textContent = t("rules.sortByColor");
    this.elements.ruleSub.textContent = `${t("hud.library")} · ${t("hud.topBook")}`;
  }
  ...
  hideFail() {
    this.hideOverlay(this.elements.failOverlay);
  }
  showPauseMenu() {
  ...
  destroy() {
    ...
    Object.values(this.elements).forEach((element) => {
      if (element?.parentNode === this.app) element.remove();
    });
    this.elements = {};
  }
}
