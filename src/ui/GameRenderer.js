...
export class GameRenderer {
  ...
  buildTable() {
    ...
    table.dataset.layoutManaged = 'true';
  ...
  buildHUD() {
    ...
    undo.type = 'button';
    undo.append('↶ ', 'Назад');
    this.cleanup.listen(undo, 'click', () => this.actions.onUndo?.());
    hud.append(undo);
    const menu = this.document.createElement('button');
    menu.className = 'menu-button';
    menu.type = 'button';
    menu.append('☰ ', 'Меню');
    this.cleanup.listen(menu, 'click', () => this.actions.onPause?.());
    hud.append(menu);
  ...
  /** Build puzzle zones; LayoutManager owns their geometry. */
  buildGameZones() {
    ...
    this.app.append(this.elements.objectsZone, this.elements.containersZone);
  }
  ...
  createOverlay(id, title, text, buttons) {
    ...
    for (const [action, label, className] of buttons) {
      ...
      this.cleanup.listen(button, 'click', () => {
  ...
  setRule(text, sub) {
    ...
    this.elements.ruleSub.textContent = sub ? `${sub} · Берём только верхнюю книгу` : 'Берём только верхнюю книгу';
  }
  ...
  renderContainers(containers, objects = []) {
    ...
    for (const container of containers) {
      ...
      for (const [index, uid] of container.items.entries()) {
        const object = byId.get(uid);
        if (!object) continue;
        const book = this.theme.renderBook(object);
        book.classList.toggle('top-book', index === 0);
        items.append(book);
      }
  ...
  /** Update top-book marker after a move. */
  updateTopBookClasses() {
    for (const shelf of this.elements.containersZone.querySelectorAll('.shelf-container')) {
      shelf.querySelectorAll('.shelf-items > .book-item').forEach((book, index) => book.classList.toggle('top-book', index === 0));
    }
  }
  ...
  showPopup(x, y, text) {
    ...
    const timeout = this.document.defaultView?.setTimeout?.(() => {
      this.timeouts.delete(timeout);
      element.remove();
    }, 700) ?? setTimeout(() => element.remove(), 700);
