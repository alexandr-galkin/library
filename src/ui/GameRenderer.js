...
export class GameRenderer {
  ...
  buildTable() {
    const table = this.document.createElement("div"); table.className = "game-table"; table.dataset.layoutManaged = "true"; this.elements.table = table; this.app.append(table);
  }
  buildHUD() {
    const hud = this.document.createElement("div"); hud.className = "hud-bar";
    const items = [[t("hud.level"), "hud-level", "1"], [t("hud.difficulty"), "hud-diff", "1"], [t("hud.moves"), "hud-moves", "0"], [t("hud.score"), "hud-score", "0"], [t("hud.time"), "hud-time", "0:00"]];
    ...
    const actions = this.document.createElement("div"); actions.className = "hud-actions";
    const undo = this.document.createElement("button"); undo.className = "menu-button undo-button"; undo.type = "button"; undo.append("↶ ", t("hud.undo")); this.cleanup.listen(undo, "click", () => this.actions.onUndo?.());
    const menu = this.document.createElement("button"); menu.className = "menu-button"; menu.type = "button"; menu.append("☰ ", t("hud.menu")); this.cleanup.listen(menu, "click", () => this.actions.onPause?.());
    actions.append(undo, menu); hud.append(actions);
    this.elements.hud = hud; this.elements.hudLevel = hud.querySelector("#hud-level"); this.elements.hudDiff = hud.querySelector("#hud-diff"); this.elements.hudMoves = hud.querySelector("#hud-moves"); this.elements.hudScore = hud.querySelector("#hud-score"); this.elements.hudTime = hud.querySelector("#hud-time");
    this.app.append(hud);
  }
  buildRuleBanner() {
  ...
  buildOverlays() {
    this.elements.failOverlay = this.createOverlay("fail-overlay", t("overlays.failTitle"), t("overlays.failText"), [["revive", `📺 ${t("overlays.revive")}`, "menu-btn-primary"], ["retry", `↻ ${t("overlays.retry")}`, "menu-btn-secondary"], ["menu", `← ${t("overlays.menu")}`, "menu-btn-secondary"]]);
  ...
  createOverlay(id, title, text, buttons) {
    ...
    const description = this.document.createElement("p"); description.textContent = text; card.append(heading, description);
  ...
  renderContainers(containers, objects = []) {
    ...
    for (const container of containers) { const element = this.document.createElement("div"); element.className = "shelf-container"; if (container.type === "empty") element.classList.add("empty-shelf"); element.dataset.id = container.id; element.dataset.capacity = String(container.capacity); const items = this.document.createElement("div"); items.className = "shelf-items";
      ...
      element.append(items); fragment.append(element);
    }
