export class GameUI {
  ...
  buildTable() { this.elements.table = this.document.createElement('div'); this.elements.table.className = 'game-table'; this.app.append(this.elements.table); }
  ...
  buildHUD() {
    ...
    undo.className = 'menu-button undo-button';
    undo.type = 'button';
    undo.append('↶ ', 'Назад');
  ...
  buildRuleBanner() {
    ...
    const title = this.document.createElement('h2'); title.id = 'rule-text';
    const subtitle = this.document.createElement('div'); subtitle.className = 'sub'; subtitle.id = 'rule-sub';
    banner.append(title, subtitle);
    this.elements.ruleBanner = banner; this.elements.ruleText = title; this.elements.ruleSub = subtitle;
    this.app.append(banner);
  }

  buildGameZones() {
    this.elements.comboDisplay = this.document.createElement('div'); this.elements.comboDisplay.className = 'combo-display';
    ...
    this.elements.containersZone = this.document.createElement('div'); this.elements.containersZone.className = 'containers-zone';
    this.app.append(this.elements.comboDisplay, this.elements.objectsZone, this.elements.containersZone);
  }

  buildOverlays() {
    this.elements.failOverlay = this.createOverlay('fail-overlay', 'НЕТ ХОДОВ', 'Откати решение или начни уровень заново', [['retry', '↻ НАЧАТЬ ЗАНОВО', 'menu-btn-primary'], ['menu', '← В МЕНЮ', 'menu-btn-secondary']]);
    this.elements.pauseOverlay = this.createOverlay('pause-overlay', '⏸ ПАУЗА', 'Головоломка приостановлена', [['resume', '▶ ПРОДОЛЖИТЬ', 'menu-btn-primary'], ['settings', '⚙ НАСТРОЙКИ', 'menu-btn-secondary'], ['menu', '← В МЕНЮ', 'menu-btn-secondary']]);
  }
  ...
  createOverlay(id, title, text, buttons) {
    ...
    const card = this.document.createElement('div'); card.className = 'modal-card';
    const heading = this.document.createElement('div'); heading.className = 'modal-title'; heading.textContent = title;
    const description = this.document.createElement('p'); description.textContent = text;
    card.append(heading, description);
    for (const [action, label, className] of buttons) {
      const button = this.document.createElement('button'); button.className = `menu-btn ${className}`; button.type = 'button'; button.dataset.action = action; button.textContent = label;
      button.addEventListener('click', () => { this.hideOverlay(overlay); this.actions[`on${action[0].toUpperCase()}${action.slice(1)}`]?.(); });
      card.append(button);
    }
  ...
  renderContainers(containers, objects = []) {
    ...
    for (const container of containers) {
      const element = this.document.createElement('div'); element.className = 'shelf-container';
      if (container.type === 'empty') element.classList.add('empty-shelf');
      element.dataset.id = container.id; element.dataset.capacity = String(container.capacity);
      const header = this.document.createElement('div'); header.className = 'shelf-header';
      const label = this.document.createElement('div'); label.className = 'shelf-label'; label.textContent = container.type === 'empty' ? 'СВОБОДНАЯ ПОЛКА' : `ПОЛКА ${container.index + 1}`;
      const count = this.document.createElement('span'); count.className = 'shelf-count'; count.textContent = `${container.items.length}/${container.capacity}`;
      header.append(label, count);
      const items = this.document.createElement('div'); items.className = 'shelf-items';
      for (const uid of container.items) { const object = byId.get(uid); if (object) items.append(this.theme.renderBook(object)); }
  ...
  moveToContainer(bookElement, containerElement, level) {
    ...
    if (!object || !source || !target) return null;
    ...
    this.placeElement(bookElement, source, target);
    return { sourceId: source.id, targetId: target.id, objectId: object.uid };
  }
  ...
  undoMove(move, level) {
    ...
    const bookElement = this.elements.containersZone.querySelector(`[data-uid="${move.objectId}"]`);
    if (!object || !source || !target || !bookElement) return false;
    target.items = target.items.filter(uid => uid !== object.uid);
    source.items.push(object.uid);
    object.shelfId = source.id;
    object.depth = source.items.length - 1;
    this.placeElement(bookElement, target, source);
    return true;
  }

  placeElement(bookElement, source, target) {
    const targetElement = this.elements.containersZone.querySelector(`[data-id="${target.id}"]`);
    const sourceElement = this.elements.containersZone.querySelector(`[data-id="${source.id}"]`);
    const targetItems = targetElement?.querySelector('.shelf-items');
    if (targetItems) targetItems.append(bookElement);
    bookElement.classList.remove('correct', 'dragging');
    targetElement?.querySelector('.shelf-count') && (targetElement.querySelector('.shelf-count').textContent = `${target.items.length}/${target.capacity}`);
    sourceElement?.querySelector('.shelf-count') && (sourceElement.querySelector('.shelf-count').textContent = `${source.items.length}/${source.capacity}`);
    targetElement?.classList.toggle('empty-shelf', target.items.length === 0);
  ...
  showPopup(x, y, text) {
    const element = this.document.createElement('div'); element.className = 'score-popup'; element.textContent = text; element.style.left = `${x}px`; element.style.top = `${y}px`; this.app.append(element);
    const timeout = setTimeout(() => { this.timeouts.delete(timeout); element.remove(); }, 700); this.timeouts.add(timeout);
  }

  destroy() { for (const timeout of this.timeouts) clearTimeout(timeout); this.timeouts.clear(); this.elements = {}; this.app.replaceChildren(); }
}
