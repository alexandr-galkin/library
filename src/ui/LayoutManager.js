...
export class LayoutManager {
  ...
  install() {
    const existing = this.document.getElementById(STYLE_ID); if (existing) { this.style = existing; return () => {}; }
    const style = this.document.createElement('style'); style.id = STYLE_ID; style.textContent = this.buildCSS(); this.document.head.append(style); this.style = style; return () => this.destroy();
  }

  /** Adapt the grid to the current number of shelves. */
  updateShelfCount(count) { const safeCount = Math.max(1, Number(count) || 1); this.document.documentElement.style.setProperty('--shelf-columns', String(Math.min(5, safeCount))); this.document.documentElement.style.setProperty('--shelf-rows', String(Math.ceil(safeCount / 5))); }

  /** Build the complete puzzle layout stylesheet from shared asset dimensions. */
  buildCSS() {
    const book = BOOK_SIZES.medium;
    return `
      :root { --game-width:830px; --game-height:640px; --game-padding:20px; --shelf-height:270px; --shelf-gap:14px; --shelf-columns:5; --shelf-rows:1; --book-width:${book.width}px; --book-height:${book.height}px; --shelf-content-height:215px; }
      ...
      @media (max-width:700px) { :root { --shelf-height:190px; --shelf-content-height:145px; --shelf-gap:4px; --book-width:${book.width}px; --book-height:${book.height}px; } #app .game-table .containers-zone { left:10px; right:10px; gap:var(--shelf-gap); } #app .game-table .shelf-container { height:var(--shelf-height); min-height:var(--shelf-height); } #app .game-table .shelf-items { padding:7px; gap:4px; } }
    `;
  }
