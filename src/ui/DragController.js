...
export class DragController {
  constructor({ getLevel, isBlocked = () => false, sound = null, onDrop = () => {}, onWrong = () => {}, root = document, validator = new BookSortDropValidator() } = {}) {
    ...
    this.cleanup.listen(this.root, 'pointerdown', this.onDown, { passive: false });
    this.cleanup.listen(this.root, 'pointermove', this.onMove, { passive: false });
    this.cleanup.listen(this.root, 'pointerup', this.onUp, { passive: false });
    this.cleanup.listen(this.root, 'pointercancel', this.onUp, { passive: false });
    this.cleanup.add(() => this.cancelFrame());
  }

  /** Pause input and cancel the active drag. */
  ...
  onPointerDown(event) {
    ...
    if (!item || item.classList.contains('correct') || !item.classList.contains('top-book')) return;
    ...
    if (!source || source.items[0] !== object.uid) return;
    ...
    if (typeof item.setPointerCapture === 'function' && this.pointerId !== null) {
      try { item.setPointerCapture(this.pointerId); } catch { /* noop */ }
    }
    this.sound?.playPick();
  }
  ...
  onPointerMove(event) {
    ...
    this.frameId = this.root.defaultView?.requestAnimationFrame?.(() => this.flushPointerFrame()) ?? requestAnimationFrame(() => this.flushPointerFrame());
  }

  /** Apply the latest pointer position and update highlights. */
  flushPointerFrame() {
    this.frameId = null;
    if (!this.isDragging || !this.dragEl || !this.latestPointer) return;
    const { x, y } = this.latestPointer;
    this.applyTransform(x, y);
    this.updateContainerHighlights(x, y);
  }
  ...
  onPointerUp(event) {
    ...
    const target = this.findTargetContainer(pointer.x, pointer.y);
    this.clearContainerHighlights();
    const object = this.dragItem;
    const element = this.dragEl;
    const level = this.getLevel();

    if (target && this.validator.canDrop(object, target.container, level)) {
      this.finishVisualDrag(element);
      this.clearDragState();
      this.onDrop(object, target.container, target.element);
      return;
    }
  ...
  /** Move the dragged element using transform rather than per-frame left/top writes. */
  ...
  cancelDrag() {
    ...
    this.releasePointer();
    this.cancelFrame();
    if (element) this.finishVisualDrag(element);
    this.clearContainerHighlights();
    this.clearDragState();
  }
  ...
  releasePointer() {
    if (this.dragEl && this.pointerId !== null && typeof this.dragEl.releasePointerCapture === 'function') {
      try { this.dragEl.releasePointerCapture(this.pointerId); } catch { /* noop */ }
    }
    this.pointerId = null;
  }
  ...
  cancelFrame() {
    if (this.frameId === null) return;
    const cancel = this.root.defaultView?.cancelAnimationFrame ?? cancelAnimationFrame;
    cancel(this.frameId);
    this.frameId = null;
  }
