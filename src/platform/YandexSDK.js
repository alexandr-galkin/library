...
async function getPlayer() { if (playerPromise) return playerPromise; const ysdk = sdk ?? await initYandexSDK(); if (!ysdk?.getPlayer) return null; playerPromise = ysdk.getPlayer({ scopes: false }).catch(error => { console.warn('[Yandex SDK] player init failed:', error); playerPromise = null; return null; }); return playerPromise; }
